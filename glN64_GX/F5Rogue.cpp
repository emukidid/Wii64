 ...
 *
 * Biggest changes: Endianness and the fact we have no GraphicsDrawer 
 * and that gDPTextureRectangle() here takes no flip argument.
**/
...
#ifndef _BIG_ENDIAN
# define SWRS_BY(i)		((i) ^ 3)
#else
# define SWRS_BY(i)		(i)
#endif

#ifndef _BIG_ENDIAN
# define SWRS_RGBA(i)	(3 - (i))
#else
# define SWRS_RGBA(i)	(i)
#endif

#ifndef _BIG_ENDIAN
# define SWRS_LANE(i)	(i)
#else
# define SWRS_LANE(i)	((i) ^ 1)
#endif
...
static bool isRDRAMRangeValid( u32 _address, u32 _size )
{
	return (_size <= RDRAMSize) && (_address <= RDRAMSize - _size);
}

#define _FIXED2FLOATCOLOR( v, b ) ((f32)(v) * 0.0039215689f)
...
static SWRSTriangle TriGen0000_defaultTriangleOrder[32] = {
...
static SWRSTriangle TriGen0001_defaultTriangleOrder[8] = {
...
static void _SWVertex( const SWVertex * vertex, u32 n )
{
	if (n > SP_VERTEX_COUNT)
		n = SP_VERTEX_COUNT;

	for (u32 i = 0; i < n; ++i)
	{
		gSP.vertices[i].x = vertex[i].x;
		gSP.vertices[i].y = vertex[i].y;
		gSP.vertices[i].z = vertex[i].z;
		gSP.vertices[i].w = 1.0f;
		gSP.vertices[i].flag = 0;

		gSPProcessVertex( i );

		gSP.vertices[i].y = -gSP.vertices[i].y;
	}
}

inline void _updateF5DL()
{
	// Lemmy's note:
...
static void F3DSWRS_PrepareVertices(const u32* _vert,
							 const u8* _colorbase,
							 const u32* _colorIdx,
							 const u8* _texbase,
							 bool _useTex,
							 ...
							 u32 _num)
{
	...
	for (u32 i = 0; i < _num; ++i) {
		SPVertex & vtx = gSP.vertices[_vert != NULL ? _vert[i] : i];
		const u8 *color = _colorbase + _colorIdx[i];
		vtx.r = _FIXED2FLOATCOLOR(color[SWRS_RGBA(0)], 8 );
		vtx.g = _FIXED2FLOATCOLOR(color[SWRS_RGBA(1)], 8 );
		vtx.b = _FIXED2FLOATCOLOR(color[SWRS_RGBA(2)], 8 );
		vtx.a = _FIXED2FLOATCOLOR(color[SWRS_RGBA(3)], 8 );
...
static void TriGen0000_PrepareColorData(const u32 * _params, std::vector<u32> & _data)
{
	const u32 dataAddr = _SHIFTR(_params[3], 0, 24);
	if (!isRDRAMRangeValid(dataAddr, 25 * sizeof(u32)))
		return;
	memcpy(&_data[0], RDRAM + dataAddr, 25 * sizeof(u32));
}

static void TriGen0001_PrepareColorData(const u32 * _params, std::vector<u32> & _data)
{
	...
	u32 idx32 = 0;
	do {
		memcpy(&word, data8 + (idx32++) * sizeof(u32), sizeof(word));
...
static void TriGen0000_PrepareVtxData(const u32 * _params, std::vector<u16> & _data)
{
	...
	for (u32 i = 0; i < 25; ++i) {
		const s8 val = rdram_data8[SWRS_BY(idx8)];
		++idx8;
...
static void TriGen0001_PrepareVtxData(const u32 * _params, std::vector<u16> & _data)
{
...
inline void TriGen00_correctVertexForOption(std::vector<u16> & _vtxData, u32 _src1, u32 _src2, u32 _dst)
{
	const s16 vd = (s16)_vtxData[_src1] + (s16)_vtxData[_src2];
	_vtxData[_dst] = (u16)(vd / 2);
}

inline void TriGen00_correctColorForOption(u32* _pColor, u32 _src1, u32 _src2, u32 _dst)
{
...
inline void TriGen00_correctVertexForPonderation(std::vector<u16> & _vtxData, u32 _src1, u32 _src2, u32 _dst, u16 ponderation)
{
	const s16 * vtxData = (const s16*)&_vtxData[0];
	s32 vd = vtxData[_dst] * (0x10000 - ponderation);
	vd += ((vtxData[_src1] + vtxData[_src2]) * ponderation) >> 1;
	_vtxData[_dst] = _SHIFTR(vd, 16, 16);
}

inline void TriGen00_correctColorForPonderation(u32* _pColor, u32 _src1, u32 _src2, u32 _dst, u16 ponderation)
{
	u8* pSrc1 = (u8*)(_pColor + _src1);
	u8* pSrc2 = (u8*)(_pColor + _src2);
	u8* pDst =  (u8*)(_pColor + _dst);
...
static void TriGen0000_option_00000100(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData, std::vector<SWRSTriangle> & _triangles)
{
	...
	// Correct color data
	TriGen00_correctColorForOption(&_colorData[0], 0, 2, 1);
	TriGen00_correctColorForOption(&_colorData[0], 2, 4, 3);
...
static void TriGen0000_ponderation1(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 0, 2, 1, ponderation1);
	TriGen00_correctColorForPonderation(&_colorData[0], 2, 4, 3, ponderation1);
}

static void TriGen0000_option_00000001(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData, std::vector<SWRSTriangle> & _triangles)
{
	...
	// Correct color data
	TriGen00_correctColorForOption(&_colorData[0], 20, 22, 21);
	TriGen00_correctColorForOption(&_colorData[0], 22, 24, 23);
...
static void TriGen0000_ponderation2(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 20, 22, 21, ponderation2);
	TriGen00_correctColorForPonderation(&_colorData[0], 22, 24, 23, ponderation2);
}

static void TriGen0000_option_01000000(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData, std::vector<SWRSTriangle> & _triangles)
{
	...
	// Correct color data
	TriGen00_correctColorForOption(&_colorData[0], 0, 10, 5);
	TriGen00_correctColorForOption(&_colorData[0], 10, 20, 15);
...
static void TriGen0000_ponderation3(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 0, 10, 5, ponderation3);
	TriGen00_correctColorForPonderation(&_colorData[0], 10, 20, 15, ponderation3);
}

static void TriGen0000_option_00010000(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData, std::vector<SWRSTriangle> & _triangles)
{
	...
	// Correct color data
	TriGen00_correctColorForOption(&_colorData[0], 4, 14, 9);
	TriGen00_correctColorForOption(&_colorData[0], 14, 24, 19);
...
static void TriGen0000_ponderation4(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 4, 14, 9, ponderation4);
	TriGen00_correctColorForPonderation(&_colorData[0], 14, 24, 19, ponderation4);
}
...
void TriGen0000_ponderation5(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 0, 12, 6, ponderation5);
	TriGen00_correctColorForPonderation(&_colorData[0], 2, 12, 7, ponderation5);
	TriGen00_correctColorForPonderation(&_colorData[0], 2, 14, 8, ponderation5);
	TriGen00_correctColorForPonderation(&_colorData[0], 10, 12, 11, ponderation5);
	TriGen00_correctColorForPonderation(&_colorData[0], 12, 14, 13, ponderation5);
	TriGen00_correctColorForPonderation(&_colorData[0], 10, 22, 16, ponderation5);
	TriGen00_correctColorForPonderation(&_colorData[0], 12, 22, 17, ponderation5);
	TriGen00_correctColorForPonderation(&_colorData[0], 12, 24, 18, ponderation5);
}

static void TriGen0001_option_01010201(const u32 * _params,
...
static void TriGen0001_ponderation1(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	const u16 ponderation1 = _SHIFTR(_params[6], 0, 16);
	if (ponderation1 == 0)
		return;
...
static void TriGen0001_option_01010102(const u32 * _params,
								std::vector<u16> & _vtxData,
								std::vector<u32> & _colorData,
								std::vector<SWRSTriangle> & _triangles)
{
	...
	// Correct color data
	TriGen00_correctColorForOption(&_colorData[0], 6, 8, 7);
...
static void TriGen0001_ponderation2(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	const u16 ponderation2 = _SHIFTR(_params[7], 16, 16);
	if (ponderation2 == 0)
		return;
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 6, 8, 7, ponderation2);
}

static void TriGen0001_option_02010101(const u32 * _params,
								std::vector<u16> & _vtxData,
								std::vector<u32> & _colorData,
								std::vector<SWRSTriangle> & _triangles)
{
	...
	// Correct color data
	TriGen00_correctColorForOption(&_colorData[0], 0, 6, 3);
...
static void TriGen0001_ponderation3(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	const u16 ponderation3 = _SHIFTR(_params[5], 0, 16);
	if (ponderation3 == 0)
		return;
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 0, 6, 3, ponderation3);
}

static void TriGen0001_option_01020101(const u32 * _params,
								std::vector<u16> & _vtxData,
								std::vector<u32> & _colorData,
								std::vector<SWRSTriangle> & _triangles)
{
	...
	// Correct color data
	TriGen00_correctColorForOption(&_colorData[0], 2, 8, 5);
...
static void TriGen0001_ponderation4(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	const u16 ponderation4 = _SHIFTR(_params[6], 16, 16);
	if (ponderation4 == 0)
		return;
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 2, 8, 5, ponderation4);
}

static void TriGen0001_ponderation5(const u32 * _params, std::vector<u16> & _vtxData, std::vector<u32> & _colorData)
{
	const u16 ponderation5 = _SHIFTR(_params[5], 16, 16);
	if (ponderation5 == 0)
		return;
	...
	// Correct color data
	TriGen00_correctColorForPonderation(&_colorData[0], 0, 8, 4, ponderation5);
}
...
static void TriGen02_BuildVtxData(const u32 * _params, u32 * _output)
{
	Vector2  v0, v1, v2, v3;
	u16 V0 = _SHIFTR(_params[9], 0, 16);
	u16 V1 = _SHIFTR(_params[8], 0, 16);
	V1 <<= 4;
	v0.VAL(SWRS_LANE(1), _SHIFTR(_params[8], 16, 16));
	v0.VAL(SWRS_LANE(0), V1);
	v0.VAL(SWRS_LANE(3), _SHIFTR(_params[9], 16, 16));
	v2.VAL(SWRS_LANE(1), V0);
	v3.VAL(SWRS_LANE(3), V0);
	v1.VAL(SWRS_LANE(0), _SHIFTR(_params[1], 16, 16));
	v1 = v1.ADD(v0);
	_output[0] = v1.data.vec32[0];
	_output[1] = v1.data.vec32[1];
	v1 = v2;
	v1.VAL(SWRS_LANE(0), _SHIFTR(_params[1], 0, 16));
	v1 = v1.ADD(v0);
	_output[2] = v1.data.vec32[0];
	_output[3] = v1.data.vec32[1];
	v1 = v3;
	v1.VAL(SWRS_LANE(0), _SHIFTR(_params[2], 16, 16));
	v1 = v1.ADD(v0);
	_output[4] = v1.data.vec32[0];
	_output[5] = v1.data.vec32[1];
	v1 = v2.ADD(v3);
	v1.VAL(SWRS_LANE(0), _SHIFTR(_params[2], 0, 16));
	v1 = v1.ADD(v0);
	_output[6] = v1.data.vec32[0];
	_output[7] = v1.data.vec32[1];
}

static void TriGen00_BuildVtxData(const u32 * _params, u32 _step, const std::vector<u16> & _input, std::vector<u32> & _output)
{
	u16 V0 = _SHIFTR(_params[9], 0, 16);
	u16 V1 = _SHIFTR(_params[8], 0, 16);
	V1 <<= 4;
	...
	for (u32 startDataIdx = 0; startDataIdx < _step; ++startDataIdx) {
		for (u32 idx = 0; idx < bound; idx += _step) {
			v1.VAL(SWRS_LANE(0), _input[(startDataIdx + idx) ^ 1]);
		...
		v1 = v1.ADD(v2);
		v1.VAL(SWRS_LANE(3), 0);
	}
}

static void TriGen00_BuildColorIndices(u32 _step, u32 _vtxSize, std::vector<u32> & _colorIndices)
{
	u32 startColorIdx = 0;
	u32 colorIdx = 0;
...
static void TriGen00_BuildTextureCoords(u32 _step, const u32 * _params, std::vector<u32> & _texCoords)
{
	...
	for (u32 i = 0; i < _step; ++i) {
		texCoord.w[SWRS_LANE(0)] = tex * (_step - 1);
		texCoord.w[SWRS_LANE(1)] = tex * i;
		for (u32 j = 0; j < _step; ++j) {
...
inline int screenZ(const SPVertex & _v)
{
	return int(((_v.z / _v.w) * gSP.viewport.vscale[2] + gSP.viewport.vtrans[2])*32768.0f);
}

static int calcAvrgZ(const SWRSTriangle & t)
{
	int zsum = screenZ(gSP.vertices[t.V0]);
	zsum += screenZ(gSP.vertices[t.V1]);
	zsum += screenZ(gSP.vertices[t.V2]);
	return zsum / 3;
}

static void TriGen0000()
{
	...
	const SWVertex * vertex = (const SWVertex*)&vtxData32[0];
	const u32 vtxSize = (u32)vtxData32.size() / 2;
	_SWVertex(vertex, vtxSize);
	...
	F3DSWRS_PrepareVertices(NULL, (u8*)&colorData[0],
							&colorIndices[0], (u8*)&texCoords[0], true, false, vtxSize);

	// Step 5. Prepare triangles, sort them by z and draw.

	for (u32 i = 0; i < triangles.size(); ++i)
		triangles[i].avrgZ = calcAvrgZ(triangles[i]);

	for (u32 i = 0; i < 32; ++i) {
		const SWRSTriangle & t = TriGen0000_defaultTriangleOrder[i];
		triangles.push_back(t);
		triangles.back().avrgZ = calcAvrgZ(t);
	}
	std::stable_sort(triangles.begin(), triangles.end(), SWRSTriangleCompare);

	for (u32 i = 0; i < triangles.size(); ++i)
		gSPTriangle(triangles[i].V0, triangles[i].V1, triangles[i].V2);
	OGL_DrawTriangles();
}

static void TriGen0001()
{
	const u32* params = (const u32*)&RDRAM[RSP.PC[RSP.PCi]];
...
static void TriGen02()
{
	...
	SPVertex & vtx2 = gSP.vertices[v2];
	SPVertex & vtx3 = gSP.vertices[v3];

	if (vtx3.z / vtx3.w > vtx2.z / vtx2.w) {
		gSPTriangle(v1, v2, v4);
		gSPTriangle(v1, v4, v3);
	} else {
		gSPTriangle(v1, v4, v3);
		gSPTriangle(v1, v2, v4);
	}
...
static void _addVertices(const u32 _vert[3], u32 _w1)
{
	SPVertex & vtx0 = gSP.vertices[_vert[(((_w1 >> 24) & 3) % 3)]];
	const f32 flat_r = vtx0.r;
	const f32 flat_g = vtx0.g;
	const f32 flat_b = vtx0.b;
	const f32 flat_a = vtx0.a;

	for (u32 i = 0; i < 3; ++i) {
		SPVertex & vtx = gSP.vertices[_vert[i]];
	...
	gSPTriangle(_vert[0], _vert[1], _vert[2]);
}
...
void F3DSWRS_Tri(u32 _w0, u32 _w1)
{
	...
	_addVertices(vert1, _w1);
	if (bTri2) {
		const u32 vert2[3] = { v1, v3, v4 };
		_addVertices(vert2, _w1);
	}
...
void F3DSWRS_MoveWord(u32 _w0, u32 _w1)
{
	...
	case G_MW_SEGMENT:
		...
		break;
	// Upstream keeps fog as floats, we don't
	case F3DSWRS_MW_FOG_MULTIPLIER:
		gSPFogFactor( (s16)_SHIFTR( _w1, 16, 16 ), gSP.fog.offset );
		break;
	case F3DSWRS_MW_FOG_OFFSET:
		gSPFogFactor( gSP.fog.multiplier, (s16)_SHIFTR( _w1, 16, 16 ) );
		break;
	case G_MW_PERSPNORM:
...
void F3DSWRS_TexrectGen(u32 _w0, u32 _w1)
{
	...
	if (vtxIdx >= SP_VERTEX_COUNT)
		return;

	const SPVertex & v = gSP.vertices[vtxIdx];
	if (v.w <= 0.0f)
		return;
	...
	const u32 w_i = MAX( (u32)1, (u32)(v.w) );
	...
	float intpart;
	const f32 frac_x_f = fabsf(modff(gSP.matrix.combined[0][0], &intpart));
	const u32 combMatrixFracX = (u32)(frac_x_f*65536.0f);
	const f32 frac_y_f = fabsf(modff(gSP.matrix.combined[0][1], &intpart));
	const u32 combMatrixFracY = (u32)(frac_y_f*65536.0f);
	param4X |= combMatrixFracX;
	param4Y |= combMatrixFracY;

	if (flip) {
		const u32 tmp = offset_x_i;
		offset_x_i = offset_y_i;
		offset_y_i = tmp;
	}
	...
	// Upstream passes flip as an argument to gDPTextureRectangle(); our gDPTextureRectangle() reads RSP.cmd instead.
	const u32 cmd = RSP.cmd;
	RSP.cmd = flip ? G_TEXRECTFLIP : G_TEXRECT;
	gDPTextureRectangle(ulx, uly, lrx, lry, gSP.texture.tile,
						_FIXED2FLOAT((s16)S, 5), _FIXED2FLOAT((s16)T, 5), dsdx, dtdy);
	RSP.cmd = cmd;
}
...
void F5Rogue_Init()
{
	...
	memset( RSP.F5DL, 0, sizeof( RSP.F5DL ) );
	F3DSWRS_ViewportAddress = 0;
	F3DSWRS_PerspMatrixAddress = 0;
	gSP.textureCoordScale[0] = 0;
	gSP.textureCoordScale[1] = 0;
