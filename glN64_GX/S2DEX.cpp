...
static u32 gs_s2dexVersion = S2DEX_VER_1_7;

void S2DEX_SetVersion( u32 version )
{
	gs_s2dexVersion = version;
}

u32 S2DEX_GetVersion()
{
	return gs_s2dexVersion;
}
...
void S2DEX_RDPHalf_0( u32 w0, u32 w1 )
{
	if (RSP.nextCmd == G_SELECT_DL)
	{
		...
		gSP.selectDL.sid  = (u8)_SHIFTR( w0, 18, 8 );
...
#ifdef SHOW_DEBUG
	sprintf( txtbuffer, "S2DEX: RDPHALF_0 nextCmd=0x%02X", (unsigned int)RSP.nextCmd );
...
{
	if (_SHIFTR( w0, 0, 8 ) == G_MW_GENSTAT)
