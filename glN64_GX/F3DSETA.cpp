/**
 * glN64_GX - F3DSETA.cpp
 *
 * RSP SW Version: 2.0D, 04-01-96
 *
 * All credit goes to https://github.com/gonetz/GLideN64/blob/master/src/uCodes/F3DSETA.cpp
**/
...
// The selector lives in bits 8..15 here, not 0..7, and the values are their own
...
static void F3DSETA_MoveWord( u32 w0, u32 w1 )
{
	switch (_SHIFTR( w0, 8, 8 ))
	{
		case F3DSETA_MW_NUMLIGHT:
			if (w1 >= 0x80000020)
				...
				DebugMsg( DEBUG_MEDIUM | DEBUG_HIGH | DEBUG_ERROR, "// F3DSETA_MW_NUMLIGHT: invalid light address 0x%08x\n", w1 );
#endif
			break;
...
#ifdef DEBUG
		default:
			DebugMsg( DEBUG_MEDIUM | DEBUG_HIGH | DEBUG_UNKNOWN, "// F3DSETA unknown MoveWord 0x%02X\n", _SHIFTR( w0, 8, 8 ) );
			break;
#endif
	}
}

static void F3DSETA_Perpnorm( u32 w0, u32 w1 )
{
